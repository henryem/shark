 ...
 */
class ReduceSinkOperator extends SimpleUnaryOperator[HiveReduceSinkOperator] {
  override def getTag() = hiveOp.getConf().getTag()
  
  override def makePartitionProcessor(): PartitionProcessor = {
    return new ReduceSinkOperator.ReduceSinkPartitionProcessor(
        hiveOp.getConf(),
  ...
  def initializeDownStreamHiveOperator() {
    ...
    val conf = hiveOp.getConf()

    // Determine output object inspector (a struct of KEY, VALUE).
    val keySer = ReduceSinkOperator.makeKeySerde(conf)
    val valueSer = ReduceSinkOperator.makeValueSerde(conf)
...
object ReduceSinkOperator {
  private class ReduceSinkPartitionProcessor(
      private val conf: ReduceSinkDesc,
      private val objectInspector: SerializableObjectInspector[ObjectInspector])
      extends PartitionProcessor {
    override def processPartition(split: Int, iter: Iterator[_]): Iterator[_] = {
      val rowInspector = objectInspector.value
      ...
      val keySer = makeKeySerde(conf)
      val valueSer = makeValueSerde(conf)
  
      // Initialize object inspector for key columns.
      val keyObjInspector = SharkEnvSlave.objectInspectorLock.synchronized {
      ...
      val valObjInspector = SharkEnvSlave.objectInspectorLock.synchronized {
      ...
      //HACK: These nested function definitions make things a bit hard to read.
      // Should refactor at some point to have an explicit state object.
      /**
  ...
  private def makeKeySerde(conf: ReduceSinkDesc): SerDe = {
  ...
  private def makeValueSerde(conf: ReduceSinkDesc): SerDe = {
